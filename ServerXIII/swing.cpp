...
auto swingGait(aris::dynamic::Model &model, const aris::dynamic::PlanParamBase &param_in)->int
{
    ...
    const double s = -PI * std::cos(PI * (param.count + 1) / param.totalCount) + PI;//s 从0到2*PI.

    if (0 != param.xAngle)//秋千平面法线沿x方向
    {
        Peb[0] = 0;
        Peb[1] = param.rDistance * (1 - std::cos(param.xAngle * sin(s)));
        Peb[2] = param.rDistance * std::sin(param.xAngle * sin(s));
        Peb[3] = 0;
        Peb[4] = -param.xAngle * std::sin(s);
        Peb[5] = 0;
    ...
    {
        Peb[0] = param.rDistance * std::sin(param.zAngle * std::sin(s));
        Peb[1] = param.rDistance * (1 - std::cos(param.zAngle * std::sin(s)));
        Peb[2] = 0;
        Peb[3] = param.zAngle * std::sin(s);
        Peb[4] = 0;
        Peb[5] = 0;
        robot.SetPeb(Peb, beginMak);
        robot.SetPee(Pee, beginMak);
